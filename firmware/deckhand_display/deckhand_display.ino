...
// ---------- Fonts ----------
// The entire UI renders in Cozette, one hand-hinted bitmap font, for a single
// crisp pixel look across every screen (a downscaled vector font goes fuzzy at
// this panel's DPI). Call sites still pass the old TFT_eSPI font numbers, but
// the number now selects only a SIZE: the legacy large font 4 -> Cozette 2x
// (26px, for hero numbers), everything else -> Cozette 1x (13px). setFreeFont
// makes drawString/textWidth use the GFX glyphs (it also sets textfont=1, which
// the free-font draw path requires). FONT_CODE stays as the "this is a code
// block" marker for panel styling, but renders in the same Cozette 1x now.
// (Kept here, after the enums, so it isn't the file's first function definition -
// see the enum note above about auto-prototype insertion order.)
const uint8_t FONT_CODE = 200;
inline uint8_t uiTextSize(uint8_t f) { return f == 4 ? 2 : 1; }
inline int uiLineH(uint8_t f) { return 13 * uiTextSize(f); }
void setUIFont(uint8_t f) {
  tft.setFreeFont(&Cozette6x13);
  tft.setTextSize(uiTextSize(f));
}
...
void drawWorkingSpinner(int cx, int cy, int r, uint16_t bg) {
  ...
  const uint8_t* f = SPARK_BITS[animPhase % SPARK_FRAMES];
...
void drawStatusDot(int cx, int cy, int r, const char* status, uint16_t bg = COLOR_BG) {
  ...
  if (strcmp(status, "working") == 0) { drawWorkingSpinner(cx, cy, r, bg); return; }
...
void drawCardChrome(int y0, const char* label) {
  ...
  tft.setTextColor(COLOR_LABEL, COLOR_CARD);
  tft.setTextDatum(TL_DATUM);
  tft.drawString(label, CARD_X + PAD, y0 + 6);   // usage cards are 122 tall - own inset
}
...
void renderCard(int y0, int pct, unsigned long tokens, long resetInMin, long windowMin,
  ...
  drawBigNumber(pctCache, 8, buf, CARD_X + PAD, y0 + 20, CARD_W - 2 * PAD, 54,
  ...
  drawPaceBar(barCache, CARD_X + PAD, y0 + 78, CARD_W - 2 * PAD, BAR_H, pct, tickPct, color);
  ...
  drawIfChanged(resetAtCache, 14, buf, CARD_X + CARD_W - PAD, y0 + 107, 1, 1,
  ...
  if (buf[0]) {
    ...
    drawIfChanged(fableCache, 24, buf, CARD_X + PAD, y0 + 107, 1, 1, COLOR_ACCENT, COLOR_CARD);
  }
}
...
void tickWorkingSpinner() {
  ...
  for (int i = 0; i < sessionCount; i++) {
    ...
    drawWorkingSpinner(SESSION_ROW_X + 20, dotCy, large ? 9 : 7, COLOR_CARD);
  }
}
...
const int P2_BTN_H = 46;
const int P2_CAL_Y = PAGE_TOP + 12;
const int P2_PAIR_Y = P2_CAL_Y + P2_BTN_H + 12;
const int P2_PWR_Y = P2_PAIR_Y + P2_BTN_H + 12;
...
void drawActionsPageStatic() {
  // Three buttons, one component, escalating intent: accent = safe,
  // warn = changes pairing, bad = powers the device down.
...
void handleSettingsTouch(int sx, int sy) {
  ...
  if (settingsPage == 1) {
    ...
    // every key, and powering off interrupts the display.
