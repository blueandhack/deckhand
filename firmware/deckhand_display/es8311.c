// Espressif's ES8311 codec driver, taken from esp-bsp (Apache-2.0; licence text
// in licenses/ES8311-Apache-2.0.txt). VERBATIM apart from the board-2-only guard
// immediately below and the matching #endif at the end of the file.
//
// es8311_register_dump() IS DELIBERATELY NEVER CALLED, and that is not an
// oversight - see toneDumpCodecRegs() in audio.ino, which does the same job with
// output that actually arrives. Two reasons, and the first one alone is fatal:
// it prints with newlib printf(), and this board's sdkconfig sets
// CONFIG_ESP_CONSOLE_UART_DEFAULT with USB Serial/JTAG only as the SECONDARY
// console - secondary catches ets_printf/ESP_LOG, not stdio - so printf() lands
// on UART0's GPIO43/44 pads while the Mac is reading the USB CDC. The dump would
// print into a void and read as a dead I2C bus. Second, it wraps every read in
// ESP_ERROR_CHECK(), so the one failure a dump exists to REPORT would instead
// abort() the chip mid-report and present as a firmware crash.
//
// WHY THE GUARD. Arduino compiles every .c/.cpp in the sketch folder as its own
// translation unit regardless of what any .ino includes, so without it board 1's
// build compiles this file too - and the reference that matters is
// i2c_master_write_read_device(), which drags legacy driver/i2c.c (and its
// global constructor) into a binary that has no I2C at all. Board 1's build is
// held byte-identical on purpose, so this file must contribute exactly nothing
// to it rather than "nothing that runs". Same reasoning, at length, in
// st77922_touch.cpp - including why the test is CONFIG_IDF_TARGET_ESP32S3 out of
// "sdkconfig.h" rather than a flag from "board.h": this is a standalone
// translation unit, board.h's board-1 branch needs art headers already included,
// and CONFIG_IDF_TARGET_ESP32S3 lives in a header rather than arriving as a bare
// -D, so it must be included explicitly or the #if silently takes the board-1
// branch on BOTH boards.
#include "sdkconfig.h"

#if !defined(CONFIG_IDF_TARGET_ESP32S3)
// Board 1 (plain ESP32): its beeper is an LEDC square wave into a discrete amp
// and there is no codec on the bus. Nothing in this file applies.
#else
